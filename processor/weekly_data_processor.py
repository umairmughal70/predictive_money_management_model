...
class DataLoaderUtil:
    ...
    def initGenerateDataBkp(self):
        ...
        try:
            ...
            logging.info('Initiating data generation on {0}'.format(date.today()))
            input = {
                  "data": [
                    [
                      {
                        "CustomerID": 141056,
                        "CategoryTypeCode": "1500–2999",
                        "Week": [
                          "2021-05-15",
                          "2021-05-22",
                          "2021-06-01",
    ...
    def retrieveDataFromDb(self):
        ...
        try:
            ...
            trans_df = pd.DataFrame (np_trans_list, columns = ['BudgetID', 'TransAmount', 'TransDate', 'AllocatedAmount'])
            trans_df['BudgetID']            = trans_df['BudgetID'].astype(float)
            ...
            budget_df['BudgetID']           = budget_df['BudgetID'].astype(float)
            budget_df['CustomerID']         = budget_df['CustomerID'].astype(float)
    ...
    def generate_weekly_dataset(self,trans_df, budget_df, no_of_months_back, least_no_of_months, total_no_of_months):
        ...
        try:
            ...
            trans_df['CustomerID'] = trans_df['CustomerID'].astype(float)
            ...
            trans_df_group['AllocatedAmount'] = trans_df_group['AllocatedAmount'].astype(float)
            
            #load from weekly data
            trans_df_group_copy = trans_df_group[['CustomerID','Week','AllocatedAmount','CategoryTypeCode','Spending']]
            print(trans_df_group_copy)
            # trans_df_group_copy.to_csv('./weeklyExport.csv')
