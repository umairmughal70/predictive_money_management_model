...
logging.basicConfig(level=env.log_level('PRED_MONEY_LOG_LEVEL',logging.INFO),
                    format=env.str('PRED_MONEY_LOG_FORMAT', '[%(asctime)s] %(levelname)s [%(name)s.%(funcName)s:%(lineno)d] %(message)s'),
                    datefmt="%d/%b/%Y %H:%M:%S",
                    stream=sys.stdout)